(function() {
  ...
  Form._onSubmitCard = function(params, event) {
    ...
    if (!Schema.validateCVC(data.cvc)) {
      fieldsValid = false;
      Form._triggerFieldError(params.onError, params.fields.cardCVC, '');
    }
    if (!Schema.validateExpiry(data.exp_month, data.exp_year)) {
      fieldsValid = false;
      if (params.fields.cardExp) {
        Form._triggerFieldError(params.onError, params.fields.cardExp, '');
      }
      if (params.fields.cardExpMonth) {
        Form._triggerFieldError(params.onError, params.fields.cardExpMonth, '');
      }
      if (params.fields.cardExpYear) {
        Form._triggerFieldError(params.onError, params.fields.cardExpYear, '');
      }
    }
    if (!Schema.validateCardNumber(data.number) || !data.number) {
      fieldsValid = false;
      Form._triggerFieldError(params.onError, params.fields.cardNumber, '');
    }
    if (!fieldsValid) {
      return;
    }
    ...
    Schema.card.createToken(data, function(status, response) {
      if (response.errors) {
        ...
        for (var key in response.errors) {
          ...
          switch (key) {
            case 'exp_month':
              field = params.fields.cardExp || params.fields.cardExpMonth;
              break;
            case 'exp_year':
              field = params.fields.cardExp || params.fields.cardExpYear;
              break;
            case 'cvc':
              field = params.fields.cardCVC;
              break;
            case 'number':
            default:
              field = params.fields.cardNumber;
              break;
          }
          Form._triggerFieldError(params.onError, field, response.errors[key].message);
        }
      } else if (status > 200) {
        Form._triggerFieldError(
          params.onError,
          params.fields.cardNumber,
          'Unknown gateway error, please try again',
        );
      } else {
